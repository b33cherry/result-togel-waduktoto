...
class Database:
    ...
    async def sync_seed_markets(self, items: list[dict[str, Any]]) -> None:
        assert self.pool
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                for item in items:
                    await conn.execute("""
                    INSERT INTO markets(slug,name,open_time,active_days,result_digits,active)
                    ...
                    ON CONFLICT(slug) DO UPDATE SET
                        ...
                        updated_at=NOW()
                    """,
                    item["slug"], item["name"], item["open_time"],
    ...
    async def ensure_day(self, target_date: date) -> None:
        ...
        async with self.pool.acquire() as conn:
            rows = await conn.fetch(
                "SELECT id, active_days FROM markets WHERE active=TRUE"
            )
            async with conn.transaction():
                for row in rows:
    ...
    async def today_rows(self, target_date: date):
        ...
        async with self.pool.acquire() as conn:
            ...
            SELECT d.*, m.slug, m.name, m.open_time, m.active_days,
                   m.result_digits, m.active
    ...
    async def mark_pending(self, daily_id: int, message_id: int) -> None:
        ...
        async with self.pool.acquire() as conn:
            ...
            SET status='PENDING', main_message_id=$2, updated_at=NOW()
            WHERE id=$1
            """, daily_id, message_id)
    ...
    async def set_main_message(self, daily_id: int, message_id: int) -> None:
        ...
        async with self.pool.acquire() as conn:
            await conn.execute("""
            UPDATE daily_results SET main_message_id=$2, updated_at=NOW()
            WHERE id=$1
            """, daily_id, message_id)

    async def set_reminder(self, daily_id: int, message_id: int, sent_at: datetime):
        assert self.pool
        async with self.pool.acquire() as conn:
            await conn.execute("""
            UPDATE daily_results
            SET reminder_message_id=$2,last_reminder_at=$3,updated_at=NOW()
            WHERE id=$1
            """, daily_id, message_id, sent_at)

    async def clear_reminder(self, daily_id: int):
        assert self.pool
        async with self.pool.acquire() as conn:
            await conn.execute("""
            UPDATE daily_results
    ...
    async def save_result(
        self, target_date: date, slug: str, value: str,
        actor_id: int, actor_name: str, force_update: bool = False
    ):
        assert self.pool
        async with self.pool.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute("""
                UPDATE daily_results
                SET status=$3,result_value=$4,filled_by_id=$5,
                    filled_by_name=$6,filled_at=NOW(),updated_at=NOW()
                WHERE result_date=$1 AND market_id=$2
                """, target_date,row["market_id"],status,value,actor_id,actor_name)
                ...
                INSERT INTO result_logs(
                    market_id,result_date,action,old_value,new_value,actor_id,actor_name
                ) VALUES($1,$2,$3,$4,$5,$6,$7)
                """, row["market_id"],target_date,action,old_value,value,actor_id,actor_name)
    ...
    async def market(self, slug: str):
        assert self.pool
        async with self.pool.acquire() as conn:
            return await conn.fetchrow("SELECT * FROM markets WHERE slug=$1", slug)

    async def add_market(self, slug: str, name: str, open_time: str, days: list[int]):
        assert self.pool
        async with self.pool.acquire() as conn:
            return await conn.fetchrow("""
            INSERT INTO markets(slug,name,open_time,active_days,result_digits,active)
    ...
    async def update_market(
        ...
        sets=[]; vals=[]; i=1
        if name is not None:
            sets.append(f"name=${i}"); vals.append(name); i+=1
        if open_time is not None:
            sets.append(f"open_time=${i}::time"); vals.append(open_time); i+=1
        if days is not None:
            sets.append(f"active_days=${i}::jsonb"); vals.append(json.dumps(days)); i+=1
        if not sets:
            return await self.market(slug)
        vals.append(slug)
        async with self.pool.acquire() as conn:
            return await conn.fetchrow(f"""
            UPDATE markets SET {", ".join(sets)}, updated_at=NOW()
            WHERE slug=${i}
            RETURNING *
            """, *vals)
    ...
    async def set_market_active(self, slug: str, active: bool):
        ...
        async with self.pool.acquire() as conn:
            return await conn.fetchrow("""
            UPDATE markets SET active=$2,updated_at=NOW()
