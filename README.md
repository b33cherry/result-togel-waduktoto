...
## Perbaikan pada build VERIFIED

Build ini memperbaiki dua masalah runtime:

1. Nilai jam dari `markets.json` sekarang dikirim sebagai teks lalu dikonversi PostgreSQL menggunakan `::text::time`. Ini mencegah error:

```text
AttributeError: 'str' object has no attribute 'hour'
```

2. Pagination menu pasaran tidak lagi mengubah `inline_keyboard` secara langsung.

Sebelum upload, Anda dapat menjalankan:
...
Saat memperbarui GitHub, hapus file lama lalu upload seluruh isi ZIP ini agar tidak ada kode versi sebelumnya yang tertinggal.
